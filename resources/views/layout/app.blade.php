...
<html lang="{{config('app.locale')}}">
  ...
  <head>
    ...
    <title>{{config('app.name', 'CODMonitoring')}}</title>
    ...
    <script src="{{asset("customjs/commandjs.js")}}"></script>
      ...
      function getUserMenus(){
        ...
        $('#parentMenu').children().remove();
           $.ajax({
                ...
                dataType : "json",
                beforeSend: function(xhr, settings) { xhr.setRequestHeader('Authorization','Bearer ' + getToken ); },
                url:"http://192.168.1.55:8002/api/getParentMenu?system_use=COD",
                success: function( msg ) {
                  var data = jQuery.parseJSON(JSON.stringify(msg));
                  var dets = '';
                    ...
                    $.each(msg, function(key, value){
                      var parentname = value.module_description;
                            ...
                            $.ajax({
                                    ...
                                    dataType : "json",
                                    beforeSend: function(xhr, settings) { xhr.setRequestHeader('Authorization','Bearer ' + getToken ); },
                                    url:"http://192.168.1.55:8002/api/getUserMenu?system_use=COD&module_id="+value.id,
                                    success: function( msg ) {
                                      //console.log(msg);
                                      //return false;
                                      var data = jQuery.parseJSON(JSON.stringify(msg));
  ...
  <body id="page-top">
    <nav class="navbar navbar-expand navbar-dark bg-dark navbar-fixed-top">
      <a class="navbar-brand mr-1" href="#"><img src="{{url('/images/logo.png')}}" style="height: 35px;" alt="cod logo"></img></a>
      <a class="navbar-brand mr-1" href="#">COD Monitoring System</a>
      ...
      <ul class="navbar-nav ml-auto ml-md-12">
        ...
        <li class="nav-item dropdown no-arrow">
          <a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            <i class="fas fa-user-circle fa-fw" style="width:250px; text-align: right;">&nbsp;<span>Current User&nbsp;:</span><span id="user"></span>
            </i>
          </a>
